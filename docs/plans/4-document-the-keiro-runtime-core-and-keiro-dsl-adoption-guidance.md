...
- [x] Milestone 5: `keiro/README.md` finalized as a complete index. (2026-07-22 17:54Z)
- [x] Milestone 5: nine `Schema.DocRef` entries appended to `mori.dhall`; `dhall type` passes. (2026-07-22 17:54Z)
- [x] Milestone 5: symbol cross-check script passes against the keiro source tree. (2026-07-22 17:54Z)
- [x] Milestone 5: link-integrity check passes with EP-2's Kiroku and migration targets present. (2026-07-22 17:54Z)
...
- Observation: the current Mori CLI validates the working-directory project with `mori validate` and rejects the older positional form `mori validate .`.
  Evidence: the positional invocation printed `Invalid argument '.'`; the no-argument invocation returned `Configuration is valid.` before registration refresh.

- Observation: the authoritative 0.3.0.0 release surface matches the runtime sources used by these docs.
  Evidence: upstream annotated tags for `keiro`, `keiro-core`, `keiro-dsl`, and `keiro-migrations` peel to release commit `c68dcc7`, all four 0.3.0.0 Hackage tarballs respond successfully, and post-tag source drift is limited to documentation and the corrected `Keiro.version` value.
...
EP-4 is complete. The repository now contains nine indexed Keiro documents: the overview plus standards for schema ownership, runtime assembly, commands, read models, workflows, telemetry, DSL adoption, and cross-cutting gotchas. Nine `keiro-*` DocRefs type-check in `mori.dhall` and are visible after refreshing `shinzui/keiro-runtime-patterns` in Mori.

Acceptance passed against the released 0.3.0.0 source: 48 load-bearing symbol checks, all eight sibling docs indexed, all relative links resolved, one-H1/tagline/tagged-fence/Related-Patterns style checks for all nine files, Dhall type-checking, and Mori configuration validation. The corrected source contracts are explicit: hand-authored startup handles `mkEventStream` warnings, snapshots become required only after retained history is insufficient, and workflow GC is optional retention work.

The durable architecture decisions were distilled into `docs/adr/0001-separate-kiroku-keiro-and-application-schemas.md` and `docs/adr/0002-adopt-keiro-dsl-for-contracts-and-evolution.md`. EP-5 can now consume the core glossary without redefining it.
...
`docs/adr/` did not exist when implementation began, so there were no prior ADRs to consult. This plan's distillation pass seeded the first two records for the schema arrangement and DSL adoption boundary.
...
Sibling plans and ownership: this plan owns the `keiro/` directory and its `mori.dhall` block (MasterPlan Integration Point 1) and the core glossary (Integration Point 4). EP-1 owns `keiki/`; EP-2 owns the now-present `kiroku/` and `migrations/` areas; EP-5 owns `messaging/` and will link back to the glossary defined here (process managers, inbox/outbox, and messaging transports are EP-5's remit, so this plan mentions them only where the runtime surface requires it — dead-letter replay, traceparent columns, the DSL's intake/emit nodes). The documentation style contract (Integration Point 2) is restated in full in the Plan of Work so this plan stands alone.

During planning, `Keiro.version` in `keiro/src/Keiro.hs` still said `"0.1.0.0"` although the cabal version was 0.3.0.0. EP-3 corrected it before this implementation; the released tags and package registry remained the version authority used for the docs.
...
- Event streams are validated at startup, once, and the validated value is what everything else consumes: define the raw `EventStream`, then call `mkEventStream "order" orderDef` and report `Left [EventStreamWarning]` as a startup failure. The validator rejects on **any** keiki 0.2 replay-safety warning — including the four checks new in keiki 0.2: head-recoverability, inversion ambiguity, unguarded input read, and state-changing silent edges — so a transducer that was "clean" under keiki 0.1 can fail here after an upgrade; that is by design, and the fix is in the transducer, not in bypassing validation. `mkEventStreamOrThrow` is for generated definitions and fixtures with a sibling proof. `mkEventStreamUnchecked` exists *only for tests and emergency forensics*; using it in production wiring is a standards violation. Cross-link `../keiki/build-time-validation.md` for what the checks mean. This doc is where the term **validated event stream** is normatively defined for the fleet (glossary duty).
...
- **Glossary rule, normative here:** snapshots are advisory while the store retains the full replayable prefix. A snapshot (`Keiro.Snapshot`, table `keiro.keiro_snapshots`) normally only accelerates hydration, and a missing, corrupt, or shape-mismatched snapshot falls back to full replay. Once per-stream truncation hides history, however, a valid snapshot must cover the hidden prefix or hydration returns `HydrationGapDetected`. Pointer: keiro repo: `docs/user/read-models-and-projections.md`, `docs/user/snapshots.md`, and guide `docs/guides/project-read-models.md`.

Create `keiro/durable-workflows.md` — deliberately the tersest doc, an orientation plus rules. Tagline: "**Workflow journals resume deterministically; deploy each progress mechanism the workflow actually uses.**" Content: a workflow runs via `runWorkflowWith runOptions name wfId body` and is driven to suspension; its journal is an ordinary event stream named `wf:<name>-<id>` on the same log, decoded with `workflowJournalCodec`. Suspended workflows need a resume worker (`resumeWorkflowsOnce` for single passes, `runWorkflowResumeWorker`/`...Push`/`...With` for continuous operation); workflows using durable sleep need scheduled `runWorkflowTimerWorker` passes; external events complete awaitables via `signalAwakeable`; `runWorkflowGcWorker` is optional terminal-instance retention housekeeping. Options nest by lens: `defaultWorkflowResumeOptions & #runOptions .~ (defaultWorkflowRunOptions & #metrics .~ metrics)`. Rules: workflow bodies must be deterministic replay-safe code (side effects only through recorded operations); evolution of a live workflow definition goes through the DSL's `patch`/`continueAsNew` support and the diff gate (link `dsl-adoption.md`). Everything else — authoring model, awaitables, child workflows, sleep semantics — is upstream: keiro repo: `docs/user/durable-workflows.md` and `docs/guides/durable-workflows.md`.
...
- Metrics: create once with `newKeiroMetrics meter` (giving a `KeiroMetrics` of 40 instruments covering command, snapshot, projection lag, outbox/inbox, timers, dispatch, workflow) and thread it everywhere via the `#metrics` lens on `RunCommandOptions`, worker options, and workflow options — the same idiom as `runtime-assembly.md`, cross-link it. Compose `Keiro.Telemetry.kirokuEventBridge` into the Kiroku connection's `eventHandler`; it counts `KirokuEventSubscriptionDeadLettered` and then calls the delegate synchronously.
...
*Named escape hatches*, so readers reach for the sanctioned ones: `ResolveHole` (a router resolver declared as a typed hole instead of a read model); named opaque group-key derivations, which stay hand-owned and require a captured fixture; the dispatch dedup raw-SQL hole; the Kafka config hole (kind 8); and the one flagged as a footgun — `scaffold --force-generated-overwrite` bypasses only the missing-`@generated`-banner safety check and will clobber a hand-edited generated file, which is only ever needed to repair a violation of the never-edit rule.
...
Edit `mori.dhall`: append nine `Schema.DocRef` entries to the end of the current `docs` list without touching or reordering any existing entry (Integration Point 1: each plan appends only its own block). Keys and kinds:
...
**3. Index and link integrity.** Every doc is linked from the README, and every relative link resolves:
...
Acceptance: eight `INDEXED` lines, no `UNINDEXED`, and no `BROKEN`; EP-2's Kiroku and migration links now resolve.
...
Everything here is additive file creation plus one append-only edit, so every step can be re-run safely. Rewriting a doc file is harmless — content converges on the specification in Plan of Work. The `mori.dhall` edit is the only step needing care: before appending, `grep -c 'key = "keiro-' mori.dhall` — if it already prints `9`, skip the edit; if it prints something between 1 and 8, a previous session stopped mid-append, so open the file, find the last complete `keiro-*` DocRef record, and add only the missing ones (the Milestone 5 list is the authoritative order). If `dhall type` fails after an edit, the error names the offending line; the fix is always local to the appended block because nothing else was touched. If the keiro source has drifted since this plan was written (a `MISS` in check 1), the source wins: update the doc text, not the check list, unless the symbol was renamed — then update both and log it in Surprises & Discoveries. Deliverable edits are confined to `keiro/`, the EP-4 block in `mori.dhall`, this plan and its MasterPlan, and the two distilled records in `docs/adr/`.
...
- 2026-07-22 (EP-4 completion): implemented and registered the nine-document corpus, refreshed Mori, and updated the Plan of Work where released source contradicted planning assumptions. Hand-authored wiring now prefers `mkEventStream`; snapshot deletion is qualified by retained-history availability; workflow resume, timer, signal, and optional GC responsibilities are separated; and the Kiroku telemetry bridge is scoped to terminal subscription dead-letter events. Reason: dependency source and released artifacts are authoritative over the pre-implementation draft.
