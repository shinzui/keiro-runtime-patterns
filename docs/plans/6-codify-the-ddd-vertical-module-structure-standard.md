...
- [x] (2026-07-22T18:44:34Z) Milestone 3: `architecture/worked-example-conversation.md` (full Conversation slice file listing) written.
- [x] (2026-07-22T18:44:34Z) Milestone 3: eight `Schema.DocRef` entries appended to `mori.dhall`; `dhall type --quiet --file mori.dhall` passed and Mori was refreshed.
- [x] (2026-07-22T18:44:34Z) Milestone 3: all relative cross-links between the eight docs and the existing keiki, keiro, messaging, migration, and kiroku areas verified; output was `LINKS-CHECKED` with no dangling links.
- [x] (2026-07-22T18:44:34Z) Milestone 3: novice acceptance exercise performed by reconstructing the `ticket` service from only the architecture rules; all load-bearing points matched the expected six packages, generated/Holes ring, snake_case read-model vertical, integration vertical, four core suites, and migrations `test-support` surface (`NOVICE-SURFACE-OK`).
- [x] (2026-07-22T18:44:34Z) Milestone 3: ADR distillation pass created `docs/adr/0004-standardize-six-package-generated-holes-verticals.md`.
- [x] (2026-07-22T18:44:34Z) MasterPlan registry row for EP-6 flipped to Complete; MasterPlan Progress checkbox ticked.
...
EP-6 is complete. The new `architecture/` area contains eight focused standards covering the six-package boundary, vertical aggregate ring, cross-cutting allowlist, extended DSL nodes, specification/scaffolding workflow, test ownership, and a complete Conversation example. All eight `architecture-*` DocRefs type-check and are visible in Mori after the local registry refresh.

Acceptance passed three reference-source path loops with no missing files, confirmed the six hospital-capacity test suites, found no dangling links or untagged opening fences, produced `SLICE-OK`, `LINKS-CHECKED`, `DHALL-OK`, and `NOVICE-SURFACE-OK`, and found the explicit rejected-layout rulings required by V4. The implementation also corrected the regeneration workflow to call out keiro-dsl's advisory-only stale-path report.

The ADR pass created `docs/adr/0004-standardize-six-package-generated-holes-verticals.md`, promoting the six-package fleet boundary and the authoritative `Generated.*` plus `Holes` convention. Task-local verification evidence remains here. EP-9's only hard dependency is now satisfied and its blueprint can consume these exact paths and rules.


## Context and Orientation

This repository, `/Users/shinzui/Keikaku/bokuno/keiro-runtime-patterns`, is a documentation-only corpus of terse, prescriptive patterns for the keiro runtime. It contains the `keiki/`, `kiroku/`, `migrations/`, `keiro/`, `messaging/`, and now `architecture/` areas, a `mori.dhall` registry file at the repo root that makes each document discoverable to agents, and the plan documents under `docs/`. There is no application code here. The relevant durable context is `docs/adr/0002-adopt-keiro-dsl-for-contracts-and-evolution.md` (the generated-code firewall and adoption boundary) and `docs/adr/0003-pgmq-vs-kafka-transport-selection.md` (the integration transport boundary); this plan created `docs/adr/0004-standardize-six-package-generated-holes-verticals.md`. This plan is EP-6 of the MasterPlan at `docs/masterplans/1-keiro-runtime-standards-docs-and-seihou-blueprints.md`; per that plan's Integration Point 3, EP-6 owns the vertical-slice convention, and EP-9 (the service-scaffolding blueprint) hard-depends on it.
...
Registration works through `mori.dhall` at the repo root: a Dhall record with a `docs` list of `Schema.DocRef` entries (fields `key`, `kind`, `audience`, `description`, `location`), where `Schema` is the pinned remote import already at the top of the file. Per MasterPlan Integration Point 1, EP-6 owns the `architecture/` directory and appends only its own DocRef block, never reordering existing entries.

Style, per MasterPlan Integration Point 2: no YAML frontmatter; a single `#` H1 title; a bold one-line tagline directly under the H1; a one-paragraph scope statement; prescriptive, rule-first prose ("The rule is one sentence: …"); code samples always in fenced blocks with language tags, using `-- CORRECT / -- WRONG` or `-- Before / -- After` contrast pairs; relative Markdown cross-links between docs in this repo; a trailing "Related Patterns" section. The existing `keiki/` docs realize most of this (they predate the bold-tagline detail; new docs include it). The soft dependencies EP-4 (`keiro/`) and EP-5 (`messaging/`) are complete, so every runtime and messaging link in this area resolves. Cross-repo references name the repository and file in plain text rather than using invalid relative links.


## Plan of Work

All new files are created under `/Users/shinzui/Keikaku/bokuno/keiro-runtime-patterns/architecture/`; the only pre-existing registry file edited is `mori.dhall` (append-only), and the durable decision is recorded in `docs/adr/0004-standardize-six-package-generated-holes-verticals.md`, the next free ADR number at implementation time. Work proceeds in three milestones. Throughout, every path claim written into a doc must be verified against the checkouts with the loops in Concrete Steps — the standard's credibility rests on every named module existing in the reference code.
