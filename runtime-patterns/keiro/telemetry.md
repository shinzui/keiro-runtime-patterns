...
`keiro.outbox.identity.conflict` counts producer enqueues refused because a retained row under the same deterministic identity has different content. It is unlabelled and has no internal recorder: call `recordProducerEnqueueOutcome` with the `ProducerEnqueueOutcome` once, after the transaction runner returns, so SQL serialization retries and the deliberate checkpoint rollback do not multiply or lose it. A custom `KeiroMetrics` record must initialize `outboxIdentityConflict`. Alert on any non-zero rate; it means a producer mapping changed under a frozen identity.
